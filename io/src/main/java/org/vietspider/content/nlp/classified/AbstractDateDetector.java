 ...
 * Oct 6, 2009  
 */
public class AbstractDateDetector {
  ...
  protected DetachDate detectInstance(String value) {
    Date dateInstance = null;
    int idx = value.indexOf(longAgo);
    if(idx > -1 && hasTimeWord(value)) {
      dateInstance = computeDate(value.substring(idx));
      if(dateInstance != null) {
        DetachDate detachDate = new DetachDate();
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(dateInstance);
        detachDate.setDate(calendar.get(Calendar.DATE));
        detachDate.setMonth(calendar.get(Calendar.MONTH)+1);
        detachDate.setYear(calendar.get(Calendar.YEAR));
        detachDate.setHour(calendar.get(Calendar.HOUR_OF_DAY));
        detachDate.setMinute(calendar.get(Calendar.MINUTE));
        return detachDate;
      }
    }
...
//        System.out.println(" xay ra 8");
    ...
    int counter =  nodeHandler.count(value);
    if(value.indexOf("hôm qua") > -1 && counter < 10) {
      Calendar calendar = Calendar.getInstance();
      detachDate.setDate(calendar.get(Calendar.DATE) - 1);
    }
    
    if(value.indexOf("hôm kia") > -1 && counter < 10) {
      Calendar calendar = Calendar.getInstance();
      detachDate.setDate(calendar.get(Calendar.DATE) - 2);
    }
    

//    System.out.println("extracted date " + detachDate.getScrore());
//    System.out.println(value  +  " ====> count " + nodeHandler.count(value));
//    detachDate.decreScrore(nodeHandler.count(value));

//        System.out.println("end score "+detachDate.getScrore() + " / "+ value);

    if(detachDate.getScrore() < 1) {
//      System.out.println(" xay ra 9");
      return null;
    }
