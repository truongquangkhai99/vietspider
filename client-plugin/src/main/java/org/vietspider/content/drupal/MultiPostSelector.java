 ...
 */
public class MultiPostSelector {
  ...
  private DrupalSyncArticlePlugin plugin;

  public MultiPostSelector(Shell parent) {
...
//    }
    ...
    SelectionAdapter syncListener = new SelectionAdapter() {
      ...
      public void widgetSelected(SelectionEvent evt) {
        ...
        for(int i = 0; i < selectors.length; i++) {
          DrupalSyncData syncData = selectors[i].getSyncData();
...
//    XPWidgetTheme.setWin32Theme(shell);
  ...
  public void setPlugin(DrupalSyncArticlePlugin plugin) {
    this.plugin = plugin;
  }
