...
public class SingleArticleSelector extends Composite {
  ...
  private XMLDrupalConfig.Category[] categories;
  ...
  public SingleArticleSelector(Composite parent, int index) {
    ...
    butTitle.setToolTipText("Post to Drupal?");
...
//  System.out.println(" == > "+ licenseFile.getAbsolutePath());
    boolean license = LicenseVerifier.verify("drupal", licenseFile);
    ...
    butFrontpage = new Button(optionComposite, SWT.CHECK);
    butFrontpage.setText(resources.getLabel("frontpage"));
    butFrontpage.setFont(UIDATA.FONT_9);
    ...
    cboCategory.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(SelectionEvent e) {
        ...
        boolean license_ = LicenseVerifier.verify("drupal", licenseFile_);
        if(license_) return;
  ...
  public void setArticle(String id, String title, String category) {
    this.articleID = id;
    ...
    butFrontpage.setEnabled(id != null && id.trim().length() > 0);
  ...
  public void setCategories(XMLDrupalConfig.Category[] categories) {
