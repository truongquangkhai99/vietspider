...
class MultiPostSelector {
  ...
  MultiPostSelector(Shell parent) {
    ...
    SelectionAdapter syncListener = new SelectionAdapter() {
      ...
      public void widgetSelected(SelectionEvent evt) {
        ...
        boolean license = LicenseVerifier.verify("Bds", licenseFile);
        int max = license ? 100 : 5;
        int counter = 0;

        for(int i = 0; i < selectors.length; i++) {
          BdsSyncData syncData = selectors[i].getSyncData();
    ...
    Worker excutor = new Worker() {
      ...
      public void after() {
        ...
        for (int i = 0; i < selectors.length; i++) {
          ...
          selectors[i].setCategories(categories, regions);
        }
      }
    };
  ...
  public void setPlugin(BdsSyncArticlesPlugin plugin) {
    this.plugin = plugin;
  }
