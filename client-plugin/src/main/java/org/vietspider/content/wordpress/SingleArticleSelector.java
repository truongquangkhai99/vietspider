...
public class SingleArticleSelector extends Composite {
  ...
  public SingleArticleSelector(Composite parent, int index) {
    ...
    butTitle.setToolTipText("Gửi tin?");
...
//    butFrontpage.setEnabled(license);
    ...
    cboCategory.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(SelectionEvent e) {
        ...
        boolean license_ = LicenseVerifier.verify("wordpress", licenseFile_);
        if(license_) return;
...
//    if(categories == null) return;
  ...
  public WordPressSyncData getSyncData() {
    ...
    syncData.setCategoryIds(new String[]{categories[idx].getCategoryId()});
