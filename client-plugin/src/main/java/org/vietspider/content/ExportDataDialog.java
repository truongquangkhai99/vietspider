...
public class ExportDataDialog {
  ...
  public ExportDataDialog(Shell parent, String value_) {
  ...
  private void export(final String domainId, final File file) {  
    Worker excutor = new Worker() {
      ...
      public void execute() {
        String plugin = "export.import.data.plugin";
        if(stop) return;
        ...
        try {
          ...
          int idx = simpleName.indexOf(".zip");
          ...
          while(page < totalPage) {
            ...
            try {
              RWData.getInstance().save(writableFile, inputStream);
              inputStream.close();
            } finally {
            ...
            page += 10;
            writableFile = new File(file.getParentFile(), simpleName+"_"+ String.valueOf(page)+".zip");
          }
        } catch (Exception e) {
