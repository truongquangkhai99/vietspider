...
class MultiPostSelector {
  ...
  private VBulletinSyncArticlesPlugin plugin;

  MultiPostSelector(Shell parent) {
    shell = new Shell(parent, SWT.CLOSE | SWT.RESIZE | SWT.APPLICATION_MODAL);
    ApplicationFactory factory = new ApplicationFactory(shell, "JoomlaSyncArticle", getClass().getName());
...
//        System.out.println(" == > "+ licenseFile.getAbsolutePath());
        ...
        for(int i = 0; i < selectors.length; i++) {
          VBulletinSyncData syncData = selectors[i].getSyncData();
...
//    XPWidgetTheme.setWin32Theme(shell);
  ...
  public void setPlugin(VBulletinSyncArticlesPlugin plugin) {
    this.plugin = plugin;
  }
