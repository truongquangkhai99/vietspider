...
public class FastWebClient2 extends FastWebClient {
  ...
  private JDKHttpLoader jdkLoader;

  public FastWebClient2() {
    jdkLoader = new JDKHttpLoader();
  }
  
  @Override
  public HttpResponse setURL(String referer, URL url) throws Exception {
    if(jdkLoader.enabledProxy(url.toURI().toString())) {
      return null;
    }
    return super.setURL(referer, url);
  }
...
//  public boolean enabledProxy(String link) {
//    return jdkLoader.enabledProxy(link);
//  }
//  
//  public byte[] loadContentByProxy(String link) throws Exception {
//    return jdkLoader.loadContent(link);    
//  }

  public byte[] loadContent(String referer, String address, String charset) throws Exception {
    if(jdkLoader.enabledProxy(address)) {
      return jdkLoader.loadContent(address);    
    }
    ...
    try {
      ...
      if(httpResponse == null) return "no respone".getBytes();
