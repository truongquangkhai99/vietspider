...
public class GroupPopup {
  ...
  private List uiGroup;
  private ArrayList<String> list = new ArrayList<String>();

  private  final Shell shell ;

  public GroupPopup(Shell parent) {
    ...
    uiGroup = factory.createList(shell, SWT.BORDER | SWT.MULTI);
    uiGroup.setFont(UIDATA.FONT_10);
    GridData gridData = new GridData(GridData.FILL_BOTH);
    uiGroup.setLayoutData(gridData); 
...
//    XPWindowTheme.setWin32Theme(shell);
  ...
  private void add() {
    Collections.addAll(list, uiGroup.getSelection());
    shell.dispose();
  }

  public ArrayList<String> getSelected() { return list; }
  ...
  private void load() {
    Worker excutor = new Worker() {
      ...
      public void after() {
        if(elements != null) uiGroup.setItems(elements);
      }
    };
    new ThreadExecutor(excutor, uiGroup).start();
  }
}
