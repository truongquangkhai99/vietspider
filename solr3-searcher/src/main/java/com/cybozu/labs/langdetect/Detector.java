...
//    }
   ...
   * the rest is cut down.
   * 
   * @param buffer the target text to append
   */


  private StringBuilder cleaningText(StringBuilder buffer) {
    ...
    if (latinCount * 2 < nonLatinCount) {
      ...
      buffer = textWithoutLatin;
    }
    return buffer;
  }

  public String detect(String _text) throws LangDetectException {
    StringBuilder buffer = new StringBuilder();
//    text = URL_REGEX.matcher(text).replaceAll(" ");
//    text = MAIL_REGEX.matcher(text).replaceAll(" ");
    char pre = 0;
    for (int i = 0; i < Math.min(_text.length(), 300); ++i) {
      char c = NGram.normalize(_text.charAt(i));
      if (c != ' ' || pre != ' ') buffer.append(c);
      pre = c;
    }

    buffer = cleaningText(buffer);

    ArrayList<Language> probabilities = getProbabilities(buffer);
   ...
   */
  public ArrayList<Language> getProbabilities(StringBuilder builder) throws LangDetectException {
    double[] langprob = detectBlock(builder);
   ...
   * 
   */
  private double[] detectBlock(StringBuilder builder) throws LangDetectException {
    ArrayList<String> ngrams = extractNGrams(builder);
    if (ngrams.size() == 0) {
...
//      if (verbose) System.out.println("==> " + sortProbability(prob));
   ...
   * @return n-grams list
   */
  private ArrayList<String> extractNGrams(StringBuilder buffer) {
    ArrayList<String> list = new ArrayList<String>();
    NGram ngram = new NGram();
    for(int i=0;i < buffer.length();++i) {
      ngram.addChar( buffer.charAt(i));
