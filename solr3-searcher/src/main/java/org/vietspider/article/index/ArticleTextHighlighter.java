...
import org.vietspider.solr2.highlight.Point;
import org.vietspider.solr2.highlight.TextElement;
import org.vietspider.solr2.highlight.TextFragment;
import org.vietspider.solr2.highlight.Word;
 ...
 * Feb 14, 2011  
 */
public class ArticleTextHighlighter {
  ...
  public ArticleTextHighlighter(CommonSearchQuery searchQuery) {
    this(searchQuery.getPattern(), searchQuery.getHighlightStart(), searchQuery.getHighlightEnd());
  }

  public ArticleTextHighlighter(String pattern, String start, String end) {
...
//      System.out.println(word.value + " : "+ indexes[0] + " : "+ indexes[1]);
    ...
    for(int i = 0; i < points.size(); i++) {
      ...
      build(builder, text, start, point.getStart(), point.getEnd());
      start = point.getEnd();
    }
...
//    System.out.println("============>"+text + " : "+ word.value + " : "+ start);
    int [] indexes = indexOf(text, word.getValue(), start, 4);
    if(indexes[0] > -1) return indexes;
    indexes = indexOf(text.toLowerCase(), word.getValue(), start, 4);
    if(indexes[0] > -1) return indexes;
    return indexOf(lower, word.getNomark(), start, word.isEquals() ? 2 : 1);
  }
...
//        System.out.println(word + " : "+ index);
  ...
  public static void main(String[] args) {
    ...
    ArticleTextHighlighter highlighter = new ArticleTextHighlighter(pattern, "<b>", "</b>");
