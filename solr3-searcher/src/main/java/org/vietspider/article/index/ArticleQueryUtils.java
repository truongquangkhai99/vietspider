...
public class ArticleQueryUtils {
  ...
  public static SolrQuery createQuery(CommonSearchQuery baseQuery, int start) {
    ...
    ArticleSearchQuery articleQuery = (ArticleSearchQuery) baseQuery;
//    System.out.println(" =========== >"+ baseQuery.getPattern());
//    System.out.println(" =========== >"+ articleQuery.getSource());
    if(baseQuery.getPattern() == null || baseQuery.getPattern().trim().length() < 1) {
      if(articleQuery.getSource() != null 
          && articleQuery.getSource().trim().length() > 0) {
        query = query.setQuery("source:\"" + articleQuery.getSource() + "\"");
      } else {
        return null;
      }
    } else {
    ...
    query = query.setFields("id", "score", "title", "title_no_mark", "text", "text_no_mark");
    query = query.setFacetMinCount(1);
    query = query.setFacetLimit(2).setStart(start);
    if("no".equals(baseQuery.getHighlightStart())) {
      query = query.setHighlight(false);
    } else {
      query = query.setHighlight(true);
      query = query.setHighlightRequireFieldMatch(true); 
      query = query.addHighlightField("title")
          .addHighlightField("title_no_mark")
          .addHighlightField("text")
          .addHighlightField("text_no_mark");
      query = query.setHighlightFragsize(100);
      query = query.setHighlightSnippets(2);
      query = query.setHighlightSimplePost(baseQuery.getHighlightEnd());
      query = query.setHighlightSimplePre(baseQuery.getHighlightStart());
    }
    query = query.setQueryType("article_handler");
...
//    System.out.println(" ==== > " + baseQuery.getPattern());
    ...
    if(articleQuery.getSource() != null 
        && articleQuery.getSource().trim().length() > 0) {
      query = query.addFilterQuery("source:\"" + articleQuery.getSource() + "\"");
    }
...
//    System.out.println(" ============ >"+ query);
  ...
  /*private static SolrQuery analyzePrice(CommonSearchQuery baseQuery, SolrQuery query) {
    ...
    try {
      ...
      List<String> elements = CharsUtil.split2List(price, '-');
      double dmin = 0;
      double dmax = 0;
