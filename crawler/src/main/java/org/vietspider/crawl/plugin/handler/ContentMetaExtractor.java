 ...
 * May 3, 2008  
 */
public final class ContentMetaExtractor {
  ...
  public ContentMetaExtractor(NodeHandler nodeHandler,
      NodePathParser pathParser, HTMLExtractor extractor, ViDateTimeExtractor timeExtractor) {
    try {
  ...
  private boolean isCommentSource(String value) {
    value = value.trim();
    if(nodeHandler.count(value) >= 5) return false;
    return value.indexOf(')') > -1 && value.indexOf(')') > -1; 
  }
  
  private boolean isAuthor(HTMLNode node) {
  ...
  public HTMLNode extractImage(List<HTMLNode> nodes) {
    if(descIndex > 0 && descIndex < nodes.size() - 1) {
      HTMLNode node = nodes.get(descIndex - 1);
      while(node != null) {
        HTMLNode imgNode = searchImageNode(node);
  ...
  private HTMLNode searchImageNode(HTMLNode node) {
...
//    return null;
  ...
  public String extractDesc(List<HTMLNode> nodes){
    String desc = null;
    descIndex = -1;
    desc = extractDesc(nodes, 20, 200);
    if(desc == null) desc = extractDesc(nodes, 5, 20);
    if(desc == null || (desc= desc.trim()).isEmpty()) desc = "...";
    return desc;
  }
  
  private String extractDesc(List<HTMLNode> nodes, int  min, int max){
    String desc = null;
    for(int i = 0; i < nodes.size(); i++) {
      HTMLNode node = nodes.get(i);
...
//  private void extractText(StringBuilder builder, HTMLNode node) {
//    if(node == null) return;
//    
//    NodeIterator iterator = node.iterator();
//    while(iterator.hasNext()) {
...
//      nodeHandler.searchTextNode(root, list);
  ...
  private List<HTMLNode> searchTextNode(HTMLNode root, final List<HTMLNode> contents){
