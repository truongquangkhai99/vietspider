...
public class ViewerContentProposalProvider implements IContentProposalProvider {
   ...
   *            requested.
   */
  public ViewerContentProposalProvider(String[] proposals) {
    super();
   ...
   */
  public void setProposals(String[] items) { this.proposals = items; }
  ...
  private String[] matches (String[] items, String prefix) {
    ...
    for (int i = 0; i < items.length; ++i) {
      if (startsWithIgnoreCase(items[i], prefix)) matches.add(items[i]);      
    }
...
//    return string.toLowerCase().startsWith(prefix.toLowerCase());
  ...
  public void dispose() { proposals = null; }
