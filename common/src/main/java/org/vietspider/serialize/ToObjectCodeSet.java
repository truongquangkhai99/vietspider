 ...
 * May 7, 2011  
 */
public class ToObjectCodeSet extends FunctionCode {
  @Override
  public void generate(Field field, 
    ...
    try {
      ...
      if(getter != null) {
        builder.append("\t\t\t");
        builder.append("set = object.").append(getter.getName()).append("();");
        builder.append('\n');
      }
    } catch (Exception e) {
    }
    ...
    builder.append("\t\t\t");
    builder.append("if(set == null) set = new ");
    if(field.getType().isInterface()) {
    ...
    builder.append("\t\t\t");
    builder.append("XML2Object.getInstance().mapCollection(set, ");
    if(elementType != null) {
...
//    System.out.println(" da thu cai nau "+ fields[i].getType());
    builder.append("\t\t\t");
    if(method == null) {
      builder.append("object.").append(name).append("=set;");
    } else {
      builder.append("object.").append(method.getName()).append("(set);");
    }
  }
}
