 ...
 * Apr 15, 2008  
 */
class ResponseReader implements TimerWorker {
...
//        System.out.println(" hihi  da thay roi 2 " + entity);
  ...
  private byte[] readBody(Header header) throws Exception  {
  ...
  private byte[] readBody(InputStream inputStream, long bodySize) throws Exception {
...
//      System.out.println(" mat het 1 "+ (System.currentTimeMillis() - start));
  ...
  private byte[] readBody(InputStream inputStream) throws Exception {
...
//    System.out.println(" buoc phai abort " + httpEntity + " / " + inputStream+ " / "+ hashCode());
    isClose = true;
    try {
      if(httpEntity instanceof BasicManagedEntity) {
        BasicManagedEntity entity = (BasicManagedEntity) httpEntity;
